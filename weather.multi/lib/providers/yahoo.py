...
class Weather():
    ...
    #hourly - extended
        for count, item in enumerate(data['forecasts']['hourly']):
            ...
            set_property('Hourly.%i.Temperature'     % (count + 1), convert_temp(item['temperature']['now'], 'F', 'C') + TEMPUNIT)
            set_property('Hourly.%i.FeelsLike'       % (count + 1), convert_temp(item['temperature']['feelsLike'], 'F', 'C') + TEMPUNIT)
    ...
    #daily - extended (yahoo)
        for count, item in enumerate(data['forecasts']['daily']):
            ...
            set_property('Daily.%i.HighTemperature' % (count + 1), convert_temp(item['temperature']['high'], 'F', 'C') + TEMPUNIT)
            set_property('Daily.%i.LowTemperature'  % (count + 1), convert_temp(item['temperature']['low'], 'F', 'C') + TEMPUNIT)
