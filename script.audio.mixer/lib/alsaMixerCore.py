...
class alsaMixerCore:
    ...
    def __runSilent(self, aCmdline):
        ...
        log(" -D- Return code= " + str(retCode))
        log(" -D- Results: StdOut=" + repr(stdout_value))
        log(" -D- Results: StdErr=" + repr(stderr_value))
                ...
                if options.setVolume:
                    if len(args) != 1:
                        ...
                        if alsaCore.setVolume(options.mixerControl, args[0]):
                            log("Error setting volume, check values")
                        else:
                            log("Control:" + options.mixerControl + " - Volume set at " + args[0])
                elif options.queryType:
                    if alsaCore.hasVolume(options.mixerControl):
                        log("Control has volume capabilities.")
                    else:
                        log("Control does not have volume capabilities.")
                else:
                    log("Volume = " + alsaCore.getVolume(options.mixerControl))
            else:
                controls = alsaCore.getPlaybackControls()
                for aControl in controls:
                    log("Control=<" + aControl + ">")
                    log("HasVolume=<" + str(alsaCore.hasVolume(aControl)) + ">")
                    if alsaCore.hasVolume(aControl):
                        log("Volume = " + alsaCore.getVolume(aControl) + " %")
