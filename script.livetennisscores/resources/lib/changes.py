...
Counting absences only means anything against a COMPLETE list. A list that
stopped at the client's page cap, lost a page to an error, or ran out of
request budget part-way through is shorter than the truth, and every match
that fell off the end of it looks absent. Counting those absences would retire
matches that are still on court -- and the worse the network, the more of them
at once. So ``diff`` takes ``complete``, and when it is False no absence is
counted at all: the counters are carried forward untouched and no finish is
announced from a disappearance. An incomplete list can still report what it
does contain, which is why matches present in one are diffed normally.
"""
...
def diff(previous, matches, notify_break_points=False, first_poll=False,
    ...
    :param complete: whether ``matches`` is the whole live list
        (``api.Result.complete``). False means absence proves nothing, so no
        absence is counted and no finish is inferred from one.
    ...
    # Whatever is left in `previous` was tracked last poll and is not listed
    # now -- but "not listed" only means "not in play" if the list was the
    # whole list. A partial one is carried forward exactly as it was found:
    # counters untouched, nothing retired, nothing announced.
    if not complete:
        for key, before in previous.items():
            current[key] = dict(before)
