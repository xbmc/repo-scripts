...
if sys.argv[1].startswith('Location'):
    ...
    if (keyboard.isConfirmed() and keyboard.getText()):
        ...
        if locs != []:
            ...
            if selected != -1:
                ADDON.setSettingString(sys.argv[1], locs[selected])
                ADDON.setSettingString(sys.argv[1] + 'id', locids[selected])
                log('selected location: %s' % locs[selected])
        else:
            log('no locations found')
            dialog.ok(ADDONNAME, xbmc.getLocalizedString(284))
else:
    location = ADDON.getSettingString('Location%s' % sys.argv[1])
    locationid = ADDON.getSettingString('Location%sid' % sys.argv[1])
    if (not locationid) and (sys.argv[1] != '1'):
        location = ADDON.getSettingString('Location1')
        locationid = ADDON.getSettingString('Location1id')
