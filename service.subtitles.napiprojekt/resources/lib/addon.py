...
def Search(item):
    if not item["file_original_path"]:
        log("No playing file path was provided by Kodi")
        xbmcgui.Dialog().notification(__scriptname__, __language__(32003))
        return

    md5hash = timeout(get_filehash, args=(item["file_original_path"], item["rar"]), timeout_duration=15)
    if not md5hash:
        log("Could not calculate the video hash (missing, unreadable or timed out file)")
        xbmcgui.Dialog().notification(__scriptname__, __language__(32004))
        return
    log("MD5: %s" % md5hash)
    ...
    if not results:
        message_ids = {
            ...
            "rate_limited": 32005,
            "network_error": 32006,
            "invalid_response": 32007,
        }
        message_id = message_ids.get(napi_helper.last_error, 32002)
        log("Search finished without results: %s" % (napi_helper.last_error or "no_subtitles"))
        xbmcgui.Dialog().notification(__scriptname__, __language__(message_id))
        return
