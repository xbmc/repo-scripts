	...
	def onClick(self, control):
		...
		if control == 1002:
			...
			if __isXbox__ == True:
				...
				#FIXME Disable housekeeping while in now playing
				pWin.show()
				while xbmcgui.getCurrentWindowId() == 10500:
					xbmc.sleep(50)
					pass
	...
	def navigate(self, click):
		if click == 'select':
			...
			if len(self.location) <= 1:
				return # Where already at top level
			else:
				n = 0
		else:
...
#				self.setStateListDown(GrooveClass.STATE_LIST_SIMILAR_ARTISTS)
	...
	def getSimilar(self, songs):
		...
		try:
			...
			b.close() #Could not get similar items
