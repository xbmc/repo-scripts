...
if __name__ == '__main__':
    ...
    npo = NPO()
    """
    guide = start_api_retriever.json_request("https://start-api.npo.nl/epg/{0}?type=tv".format("2018-11-29"))
    print(guide)
    print('----------------------------------------------------------------------------')
    pprint.pprint([recent for recent in npo.get_recents()])
    print('----------------------------------------------------------------------------')
    pprint.pprint([duration for duration in npo.get_duration(10 * 60, 15 * 60)])
    """
    print('----------------------------------------------------------------------------')
    pprint.pprint(npo.get_guide_channels(datetime.datetime.now()))
    """
    print('----------------------------------------------------------------------------')
    pprint.pprint([broadcast for broadcast in npo.get_channel_guide("npo-1", time.time())])
    print('----------------------------------------------------------------------------')
    print(npo.get_letters())
    pprint.pprint([serie for serie in npo.get_series('D')])
    print('----------------------------------------------------------------------------')
    print(npo.get_genres())
    pprint.pprint([serie for serie in npo.get_series_by_genre('Sport')])
    print('----------------------------------------------------------------------------')
    pprint.pprint([episode for episode in npo.get_episodes('POMS_S_KN_14921953')])
    """