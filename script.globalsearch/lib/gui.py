...
class GUI(xbmcgui.WindowXML):
    ...
    def _load_settings(self):
        for key, value in CATEGORIES.items():
            if key not in ('albumsongs', 'artistalbums', 'artistsongs', 'tvshowseasons', 'tvshowepisodes', 'seasonepisodes', 'actormovies', 'directormovies', 'actortvshows'):
    ...
    def _get_preferences(self):
        ...
        if 'result' in json_response and json_response['result'] != None and 'value' in json_response['result']:
            self.selectaction = json_response['result']['value']
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "Settings.GetSettingValue", "params":{"setting":"myvideos.playaction"}, "id": 1}')
        json_response = json.loads(json_query)
        self.playaction = 1
        if 'result' in json_response and json_response['result'] != None and 'value' in json_response['result']:
            ...
            if self.selectaction == 0: #choose
                ...
                if selection >= 0:
                    if functions[selection] == 'play':
                        ...
                        self.selectaction = 3
            if self.selectaction == 3: #info
                self._show_info(listitem)
            elif self.selectaction == 8: #play
                if self.playaction == 1: #play or resume
                    if int(resume) > 0:
                        labels = ()
                        functions = ()
