...
if (xbmc.getCondVisibility('Player.HasMedia') == False):
    xbmc.executebuiltin('Notification(%s, %s, %d)'%("",Addon.getLocalizedString(30015), t))
    sys.exit()
...
if d2 == d1:
    ...
    if float(sources) == -0.000000:
        sources = "0.000000"
    xbmc.executebuiltin('Notification(%s, %s, %d)'%(format(float(sources), '.3f') + n,line2, t))
    ...
    if float(sources) == -0.000000:
        sources = "0.000000"
    xbmc.executebuiltin('Notification(%s, %s, %d)'%(format(float(sources), '.3f') + n,line1, t))
    ...
    if isinstance(y, float):
        ...
        if float(sources) == 0:
            n = Addon.getLocalizedString(30023)
        sources = sourcesXML.getElementsByTagName('audiodelay')[0].firstChild.nodeValue
        xbmc.executebuiltin('Notification(%s, %s, %d)'%(format(float(sources), '.6f') + n," ", t*6))
        ...
        if ok == True:
            ...
            sourcesXML = minidom.parse(xbmc.translatePath('special://profile/guisettings.xml'))
            sources = sourcesXML.getElementsByTagName('audiodelay')[0].firstChild.nodeValue
            xbmc.executebuiltin('Notification(%s, %s, %d)'%(format(float(sources), '.3f') + Addon.getLocalizedString(30019),"", t*6))
