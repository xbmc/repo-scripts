...
def geoip():
    ...
    if response:
        ...
        if data and 'city' in data and 'country_name' in data:
...
def location(locstr):
    ...
    loc = unicodedata.normalize('NFKD', locstr).encode('ascii','ignore')
    log('searching for location: %s' % loc)
    search_string = 'find?q=%s&type=like&APPID=%s' % (urllib.parse.quote(loc), APPID)
