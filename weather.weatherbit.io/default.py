...
def refresh_locations():
    ...
    for count in range(1, 6):
        loc_name = ADDON.getSettingString('Location%s' % count)
        if loc_name != '':
            locations += 1
        else:
            ADDON.setSettingString('Location%sdeg' % count, '')
...
def forecast(loc, locationdeg):
    ...
    if MAPS and xbmc.getCondVisibility('System.HasAddon(script.openweathermap.maps)'):
...
if not APPID:
    ...
    if (keyboard.isConfirmed() and keyboard.getText() != ''):
        ...
        if locations and locations != []:
            ...
            if selected != -1:
                ADDON.setSettingString(sys.argv[1], locations[selected].split(' - ')[0])
                ADDON.setSettingString(sys.argv[1] + 'deg', locationdeg[selected])
    ...
    locationname = ADDON.getSettingString('Location%s' % sys.argv[1])
    locationdeg = ADDON.getSettingString('Location%sdeg' % sys.argv[1])
    if (locationdeg == '') and (sys.argv[1] != '1'):
        locationname = ADDON.getSettingString('Location1')
        locationdeg = ADDON.getSettingString('Location1deg')
        log('trying location 1 instead')
    if locationdeg == '':
        log('fallback to geoip')
        locationname, locationdeg = geoip()
        if locationname and locationdeg:
            ADDON.setSettingString('Location1', locationname.encode("utf-8"))
            ADDON.setSettingString('Location1deg', locationdeg)
    if not locationdeg == '':
