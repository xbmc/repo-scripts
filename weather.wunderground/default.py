...
def recode(alert): # workaround: wunderground provides a corrupt alerts message
    try:
        alert = alert.encode("latin-1").rstrip('&nbsp)')
    except:
        pass
    return alert
...
def location(string):
    ...
    loc = unicodedata.normalize('NFKD', string).encode('ascii','ignore')
...
def forecast(loc,locid):
    ...
    data = parse_data(query)
    if data != '' and 'response' in data and not 'error' in data['response']:
...
# today properties
    ...
    if TIMEFORMAT != '/':
        AM = TIMEFORMAT.split('/')[0]
        PM = TIMEFORMAT.split('/')[1]
...
# map properties
    ...
    xbmcvfs.File(map1, 'w').write(bytearray(data1))
    xbmcvfs.File(map2, 'w').write(bytearray(data2))
    xbmcvfs.File(map3, 'w').write(bytearray(data3))
