...
def find_movie_details( movie_db=None, title="" ):
    """ find_movie_details( title )

        Searchs for a Movie Title match to the supplied 'title', returns matching movie details
    ...
    """
    xbmc.log( "[json_utils.py] - find_movie_details - Title: %s" % title, level=xbmc.LOGDEBUG )
    movie_return = None
    if not movie_db:
...
def insert_movie_into_playlist( movie_db=None, title="", index=0 ):
    ...
    matched_movie = find_movie_details( movie_db=movie_db, title=title )
    if matched_movie:
