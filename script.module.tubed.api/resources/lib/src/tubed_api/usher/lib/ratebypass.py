...
def throttling_reverse(arr):
    ...
    """
    reverse_copy = arr[::-1]
    for i, _ in enumerate(reverse_copy):
...
def js_splice(arr, start, delete_count, *items):
    ...
    Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice  # pylint: disable=line-too-long
    """
...
class CalculateN:
    ...
    MAPPING_FUNC_PATTERNS = (
        (r"{for\(\w=\(\w%\w\.length\+\w\.length\)%\w\.length;\w--;\)\w\.unshift\(\w.pop\(\)\)}", throttling_unshift),
    ...
    def get_throttling_function_code(js):
        ...
        if fiduciary_index == -1:
            LOG.debug('ratebypass: fiduciary_index not found')
            return None

        start_index = js.rfind('=function(', 0, fiduciary_index)
        if start_index == -1:
            LOG.debug('ratebypass: function code start not found')
            return None

        end_index = js.find('};', fiduciary_index)
        if end_index == -1:
            LOG.debug('ratebypass: function code end not found')
            return None
    ...
    def get_throttling_plan_gen(raw_code):
        ...
        if plan_start_index == -1:
            LOG.debug('ratebypass: command block start not found')
            raise Exception()
        ...
        if plan_end_index == -1:
            LOG.debug('ratebypass: command block end not found')
            raise Exception()
    ...
    def get_throttling_function_array(cls, mutable_n_list, raw_code):
        ...
        if array_start_index == -1:
            LOG.debug('ratebypass: "c" array pattern not found')
            raise Exception()
        ...
        if array_end_index == -1:
            LOG.debug('ratebypass: "c" array end not found')
            raise Exception()
        ...
        for el in cls.array_reverse_split_gen(array_code):
            ...
            if el.startswith('function'):
                ...
                for pattern, fn in cls.MAPPING_FUNC_PATTERNS:
                    ...
                    LOG.debug('ratebypass: mapping function not yet '
    ...
    def calculate_n(self, mutable_n_list):
        ...
        try:
            ...
            LOG.debug('Error calculating new `n`')
            return None
