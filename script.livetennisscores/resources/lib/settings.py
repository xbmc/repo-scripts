...
class Settings:
    ...
    def daily_request_limit(self):
        """Requests the add-on may spend in a UTC day.

        The free plan's 100 by default. A paid key can raise it; the clamp in
        :func:`resources.lib.budget.clamp_limit` means the setting can only
        ever raise it, so nobody can quietly configure the add-on into
        spending more than it says it does, or less than it needs to work.
        """
        return budget.clamp_limit(
            self._int("daily_requests", budget.DEFAULT_DAILY_LIMIT))
