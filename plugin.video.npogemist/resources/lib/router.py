...
class Router(object):
    ...
    def _find_callable(self, url):
        ...
        for (f, mapped_url) in self.routes.iteritems():
            if url == mapped_url:
                return f
        return None
...
if __name__ == '__main__':
    ...
    class XBMCStub(object):
        def log(self, s):
            print(s)

    base = 'plugin://plugin.video.npogemist/'
    router = Router(42, base, XBMCStub())
    ...
    print("--- Expect Dup error")
    try:
        @router.route('/')
        def index2(a=None, b=None):
            print("Hello, world! {0} {1}".format(a, b))
    except Exception as e:
        p(e)

    print("--- Expect url with params")
    print(router.make_url(index, a="a", d="c"))

    print("--- Expect round trip")
    (url, query) = router.make_url(index, a="a", b="c").split("?")
    print(url + " " + query)
    router.dispatch(url, "?" + query)
