...
class subtitles:
    ...
    def get(self, query):
        try:
            ...
            subtitles = [i for i in subtitles if '/assets/images/1.png' in i]
        except:
            return
        ...
        for subtitle in subtitles:
            try:
                ...
                url = common.parseDOM(subtitle, "a", ret="href")[-1]
    ...
    def download(self, url, name):
        try:
            ...
            result = getUrl(url).result
            url = common.parseDOM(result, "a", ret="href", attrs = { "class": "NSButton" })
            url = [i for i in url if '/download/' in i][0]

            result = getUrl(url).result

            if result[:4] == 'Rar!':
                rarfile = os.path.join(tempData,"subtitles.rar")
            elif result[:2] == 'PK':
                rarfile = os.path.join(tempData,"subtitles.zip")
            else:
                rarfile = os.path.join(tempData,"subtitles.srt")
            file = open(rarfile, 'wb')
            file.write(result)
            file.close()

            if not rarfile.endswith('.srt'):
