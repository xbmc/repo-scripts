...
class Player(xbmc.Player):
	...
	def onPlayBackStarted(self):
		if self.info is not None:
			self.info['current_time'] = num(self.getTime())
	...
	def reportPlaybackProgress(self, info, action):
		...
		if self.tracking_key is not None:
			res = controller.getListData('tracking/report_playback/?key='+self.tracking_key+'&stream_started='+str(num(info['stream_started']))+'&current_time='+str(num(info['current_time']))+'&action='+action)
...
class login:
	...
	def getData(self, url):
		...
		self.request = urllib2.Request(url, send, headers={"User-Agent" : "XBMC/Kodi BgTimeTv Addon " + str(VERSION)})
		try:
