...
class LegendasTV:
    ...
    def pageDownload(self, MainID, MainIDNumber, Page):
        ...
        if not re.findall(regex_1, Response, re.IGNORECASE | re.DOTALL):
            ...
            for x, content in enumerate(
                    re.findall(regex_1, Response, re.IGNORECASE | re.DOTALL),
                    start=1):
    ...
    def Search(self, **kargs):
        ...
        self.DownloadsResults = sorted(self.DownloadsResults,
                                       key=itemgetter('main_id_number', 'page',
                                                      'position'))
        IncludedResults = []
        ExcludedResult = []
        if TVShow:
        ...
        IncludedResults = sorted(IncludedResults,
                                 key=itemgetter('type', 'language_preference'))

        # # Log final results
        self.Log(" ")
        self.Log("Included results:")
        self._log_List_dict(IncludedResults,
                            "filename language_name language_preference type")
        self.Log("Excluded results:")
        self._log_List_dict(ExcludedResult,
                            "filename language_name language_preference type")
