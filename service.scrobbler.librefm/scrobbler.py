...
class Main:
    ...
    def _service_authenticate( self, service, timestamp ):
        ...
        # create a pass hash
        md5pass = hashlib.md5()
        md5pass.update(service[3].encode('utf-8'))
        # generate an authentication token
        md5token = hashlib.md5()
        md5token.update(md5pass.hexdigest().encode('utf-8') + timestamp.encode('utf-8'))
        ...
        try:
            ...
            result = req.read()
            req.close()
            log('%s authentication %s' % (service[0], result))
            data = result.decode("utf-8").split('\n')
            log('authentication result %s' % str(data))
        except:
        ...
        if data[0] == 'OK':
            ...
            xbmc.executebuiltin(u'Notification(%s,%s)' % ('Scrobbler: ' + service[0], LANGUAGE(32002)))
    ...
    def _service_nowplaying( self, service, tags ):
        ...
        try:
            ...
            return service
        log('%s nowplaying announce result %s' % (service[0], data[0]))
        # parse results
    ...
    def _service_submit( self, service, tstamp ):
        ...
        if not submit:
            return service
        log('%s submission data %s' % (service[0], data))
        try:
            # submit request
            body = urllib.parse.urlencode(data).encode('utf-8')
            req = urllib.request.Request(url, body)
            ...
            return service
        log('%s submit result %s' % (service[0], data[0]))
        # parse results
        if data[0] == 'OK':
            # empty our queue
            service[13] = []
