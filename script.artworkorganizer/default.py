...
def clean_filename(tmp_filename):
    ...
    filename = unicodedata.normalize('NFKD', unicode(unicode(tmp_filename, 'utf-8'))).encode('ascii','ignore')
    return filename
...
class Main:
    ...
    def _copy_moviefanart( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"fields": ["fanart", "year"] }, "id": 1}')
    ...
    def _copy_tvshowfanart( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": {"fields": ["fanart"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_musicvideofanart( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetMusicVideos", "params": {"fields": ["fanart", "artist"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_artistfanart( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "AudioLibrary.GetArtists", "params": {"fields": ["fanart"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_moviethumbs( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"fields": ["thumbnail", "year"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_tvshowthumbs( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": {"fields": ["thumbnail"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_musicvideothumbs( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetMusicVideos", "params": {"fields": ["thumbnail", "artist"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_artistthumbs( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "AudioLibrary.GetArtists", "params": {"fields": ["thumbnail"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
    ...
    def _copy_albumthumbs( self ):
        count = 0
        processeditems = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "AudioLibrary.GetAlbums", "params": {"fields": ["thumbnail", "artist"] }, "id": 1}')
        json_response = re.compile( "{(.*?)}", re.DOTALL ).findall(json_query)
        totalitems = len( json_response )
        for item in json_response:
            if self.dialog.iscanceled():
