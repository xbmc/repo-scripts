...
class EnumMeta(type):
    ...
    def __new__(metacls, cls, bases, classdict):
        ...
        if _order_ is None:
            if pyver < 3.0:
                try:
                    _order_ = [name for (name, value) in sorted(list(members.items()), key=lambda item: item[1])]
                except TypeError:
...
def _convert(cls, name, module, filter, source=None):
    ...
    members = dict((name, value) for name, value in list(source.items()) if filter(name))
