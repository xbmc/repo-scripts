...
class ConfUpdate():
    ...
    def _saveNewFile(self, dialogXml, dialogXmlStr):
        ...
        xbmcvfs.copy(dialogXml, "%s.recap-%s.bak" % (dialogXml, self.bak_timestamp))
    ...
    def _updateDialogVideoInfo(self):
        ...
        # Now check to see if the skin file has already had the recap bits added
        if 'script.recap' in dialogXmlStr:
            # Already have recap referenced, so we do not want to do anything else
            # to this file
            log("DialogVideoInfo: Recap already referenced in %s, skipping file" % dialogXml, xbmc.LOGINFO)
            self.errorToLog = True
            return
        ...
        # Now add the Recap button after the Final one
        DIALOG_VIDEO_INFO_BUTTON = '''\n\t\t\t\t\t</control>\n\t\t\t\t\t<control type="button" id="%d">
\t\t\t\t\t\t<description>Recap</description>
\t\t\t\t\t\t<include>ButtonInfoDialogsCommonValues</include>
\t\t\t\t\t\t<label>$ADDON[script.recap 32001]</label>
\t\t\t\t\t\t<onclick>RunScript(script.recap)</onclick>
\t\t\t\t\t\t<visible>System.HasAddon(script.recap) + [Container.Content(tvshows) | Container.Content(seasons) | Container.Content(episodes)]</visible>'''
...
if __name__ == '__main__':
    log("Recap: Updating Confluence Skin (version %s)" % ADDON.getAddonInfo('version'))
